...
# scripts/proxmox/homeassistant/bnu-homeassistant/scripts/waha.py — keep in sync
# (a plain `diff` against that file should show only this header). Copied rather
# than imported because a Docker build context cannot reach outside its own
# directory. This copy uses only send_text() and session_status().
