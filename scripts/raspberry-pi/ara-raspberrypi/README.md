...
| [`starlink-names/`](starlink-names/) | systemd timer (5 min): Starlink router gRPC `wifi_get_clients` → auto-nickname new devices in netoverview with the names the Starlink app shows; never overwrites manual renames. "Has a nickname" = **known device** for the presence report and the bnu HA Frigate gate |
