...
| [`timelapse/`](timelapse/) | Daily construction-timelapse frames off the local relay: 8 solar windows (sunrise T…T+20 + sunset T−20…T+20, PT lens + fixed-lens twin, NOAA per day) + worker-presence frames every 15 min 07:00–18:00 → nightly 20:00 `rclone move` to Google Drive `CeuAzul/Timelapse/`, deleting local copies on confirmed transfer (AI ground-truth for build progress; see home-ara CLAUDE.md) |
